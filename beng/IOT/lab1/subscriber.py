...
def run_subscribers():
    ...
    subscriber1 = threading.Thread(
        target=run_subscriber, args=("localhost", 1883, TOPIC_1, 0)
    )
    subscriber2 = threading.Thread(
        target=run_subscriber, args=("localhost", 1883, TOPIC_2, 1)
    )
    subscriber3 = threading.Thread(
        target=run_subscriber, args=("localhost", 1883, TOPIC_3, 2)
    )
    subscriber1.start()
    subscriber2.start()
    subscriber3.start()
