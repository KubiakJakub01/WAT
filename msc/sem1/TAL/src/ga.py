...
def total_dist_individual(config: Config, individual):
    ...
    for i in range(0, len(individual)):
        if i == len(individual) - 1:
            total_dist += dist_two_cities(config, individual[i], individual[0])
        else:
            total_dist += dist_two_cities(config, individual[i], individual[i + 1])
    return total_dist


def dist_two_cities(config: Config, city_1, city_2):
    """
    Calculating the distance between two cities
    Input:
    1- City one coordinates
    2- City two coordinates
    Output:
    Calculated Euclidean distance between two cities
    """
    return euclidean_distance(config.city_coords[city_1], config.city_coords[city_2])
