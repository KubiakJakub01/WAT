    ...
    for iteration in range(max_iter):
        ...
        for j in range(n):
            ...
            if denominator <= 0:
                ...
                print(f"Iteration {iteration}: N={N:.5f} is <= n-1 ({n-1}). Resetting N.")
        ...
        if phi <= 0 or (phi * T) == 0:
             print(f"Iteration {iteration}: Invalid intermediate value phi={phi:.10f}, T={T}. Stopping.")
        ...
        if new_N <= n - 1:
             print(f"Iteration {iteration}: Calculated new_N={new_N:.5f} <= n-1 ({n-1}). Adjusting N.")
        ...
        if abs(N - N_old) < precision:
            ...
            phi = (2.0 / T) * sum_1_div_N_minus_j
            print(f"Converged after {iteration+1} iterations with N={N:.4f} and phi={phi:.10f}")
            return N, phi
...
def calculate_E_Tk(N, phi, k):
    ...
    if N <= target_k_minus_1:
        print(f"Cannot calculate E[T_{k}]: Estimated N ({N:.4f}) <= {target_k_minus_1}.")
        return None
    if phi <= 0:
    ...
    if n_data_points > 0:
        ...
        if N_mle is not None and phi_mle is not None:
            ...
            if expected_time is not None:
                print(f"Expected time until the {target_bug_index}th bug (E[T_{target_bug_index}]): {expected_time:.4f}")
            else:
